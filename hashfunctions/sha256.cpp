...
//** we will replace BSIG0 with EP0 and BSIG1 with EP0 in our
//** implementation
...
//** (5) if we are doing test, take the result and compare it with //** expected result
...
vector<unsigned long> resizingTheBlock(vector<unsigned long> inputOf8BitAsciiValues) {
    ...
    for (int i = 0; i < 64; i = i + 4) {
        ...
        temporary32BitSection = (unsigned long)
                                    inputOf8BitAsciiValues[i]
                                << 24;
        temporary32BitSection |= (unsigned long)
                                     inputOf8BitAsciiValues[i + 1]
                                 << 16;
        temporary32BitSection |= (unsigned long)
                                     inputOf8BitAsciiValues[i + 2]
                                 << 8;
        temporary32BitSection |= (unsigned long)
...
//** get the ascii values stored as a vector in binary and add padding to it in order to obtain a total of 512 bits.
vector<unsigned long> addPadOf512Bits(vector<unsigned long> vectorBlockHoldingAsciiCharacters) {
    ...
    if (displayBlockStateAddOne)
        outputTheBlockState(vectorBlockHoldingAsciiCharacters);
    outputTheBlockState(vectorBlockHoldingAsciiCharacters);
    //** we have 7 zeroes. We will need to substract 7 from
    //** zeroesToAdd
    ...
    if (displayResultsOfPadding) {
        ...
        for (int i = 0; i < vectorBlockHoldingAsciiCharacters.size(); i = i + 4)
            cout << i << ": "
                 << "0x" + displayAsHex(vectorBlockHoldingAsciiCharacters[i]) << "     "
                 << i + 1 << ": "
                 << "0x" + displayAsHex(vectorBlockHoldingAsciiCharacters[i + 1]) << "     "
                 << i + 2 << ": "
                 << "0x" + displayAsHex(vectorBlockHoldingAsciiCharacters[i + 2]) << "     "
                 << i + 3 << ": "
                 << "0x" + displayAsHex(vectorBlockHoldingAsciiCharacters[i + 3]) << endl;
    }
    return vectorBlockHoldingAsciiCharacters;
}
...
string computingTheHash(const vector<unsigned long>
    ...
    for (int t = 0; t < 64; t++) {
        ...
        if ((t == 20) & displayT2Computation) {
            ...
            cout << "MAJ(a,b,c): 0x" << hex
                 ...
                 << (int)MAJ(a, b, c) << endl;
            cout << "temporary_2 = 0x" << hex << temporary_2 << "  dec:" << dec << temporary_2 << "  sign:" << dec << (int)temporary_2 << endl;
        }
        ...
        if (displayWorkVariablesForT) {
            ...
            cout << displayAsHex(a) << " " << displayAsHex(b)
                 << " " << displayAsHex(c) << " " << displayAsHex(d) << " " << displayAsHex(e) << " " << displayAsHex(f) << " " << displayAsHex(g) << " " << displayAsHex(h) << " " << endl;
        }
    }
    //** display the content of each of the hash segment
    if (displayTheHashSegments) {
        cout << "InitialHashValueFor32Bit_0 = " << displayAsHex(InitialHashValueFor32Bit_0) << " + " << displayAsHex(a) << " " << displayAsHex(InitialHashValueFor32Bit_0 + a) << endl;
        cout << "InitialHashValueFor32Bit_1 = " << displayAsHex(InitialHashValueFor32Bit_1) << " + " << displayAsHex(b) << " " << displayAsHex(InitialHashValueFor32Bit_1 + b) << endl;
        cout << "InitialHashValueFor32Bit_2 = " << displayAsHex(InitialHashValueFor32Bit_2) << " + " << displayAsHex(c) << " " << displayAsHex(InitialHashValueFor32Bit_2 + c) << endl;
        cout << "InitialHashValueFor32Bit_3 = " << displayAsHex(InitialHashValueFor32Bit_3) << " + " << displayAsHex(d) << " " << displayAsHex(InitialHashValueFor32Bit_3 + d) << endl;
        cout << "InitialHashValueFor32Bit_4 = " << displayAsHex(InitialHashValueFor32Bit_4) << " + " << displayAsHex(e) << " " << displayAsHex(InitialHashValueFor32Bit_4 + e) << endl;
        cout << "InitialHashValueFor32Bit_5 = " << displayAsHex(InitialHashValueFor32Bit_5) << " + " << displayAsHex(f) << " " << displayAsHex(InitialHashValueFor32Bit_5 + f) << endl;
        cout << "InitialHashValueFor32Bit_6 = " << displayAsHex(InitialHashValueFor32Bit_6) << " + " << displayAsHex(g) << " " << displayAsHex(InitialHashValueFor32Bit_6 + g) << endl;
        cout << "InitialHashValueFor32Bit_7 = " << displayAsHex(InitialHashValueFor32Bit_7) << " + " << displayAsHex(h) << " " << displayAsHex(InitialHashValueFor32Bit_7 + h) << endl;
    }
    ...
    return displayAsHex(InitialHashValueFor32Bit_0) +
           displayAsHex(InitialHashValueFor32Bit_1) + displayAsHex(InitialHashValueFor32Bit_2) + displayAsHex(InitialHashValueFor32Bit_3) + displayAsHex(InitialHashValueFor32Bit_4) + displayAsHex(InitialHashValueFor32Bit_5) +
